			...
			if repoSuccess:
				...
				if followProcessing and isinstance(linksH, dict):
					...
					if isinstance(language, str) and len(language) > 0:
						...
						agent_buildSystem = self.recognizedBuildSystemsByLang.get(lcLanguage)
						if agent_buildSystem is not None:
							ans['agent_buildSystem'] = agent_buildSystem
					...
					#			# project license
					#			$ans{'agent_license_uri'} = $licData->{'_links'}{'html'};
					#			# sourcecode license
					...
					# Versions
					if len(versions) > 0:
						ans['agent_versions'] = versions
					# credits
					if len(contributors) > 0:
						ans['agent_developers'] = contributors
