...
TabKeyOrder = [
	...
	('agent_buildSystem','type'),
	
	('agent_license_uri','project license'),
...
if __name__ == "__main__":
	...
	if (jsondir is not None) or (tabfile is not None):
		...
		if isinstance(p_queries, Iterable):
				...
				for numRes, query in enumerate(query_l):
					...
					for fullrepo in fullans['repos']:
						...
						if TAB is not None:
							...
							# Synthetic
							p_ans.setdefault('agent_id', agent_id)
					...
					if jsondir is not None:
						subRel = '{0:06d}'.format((numAgent // DirMaxSize) * DirMaxSize)
						partialJsonout = f'agent-{numAgent:06d}_{numRes}.json'
						if ext is not None:
							partialJsonout += ext
			...
			print(f"* Processed {numAgent} agents")
