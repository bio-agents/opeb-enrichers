...
if(defined($jsondir) || defined($tabfile)) {
	...
	if(scalar(@{$p_queries})>0) {
		...
		print "* Processing ".scalar(@{$p_queries})." agents\n";
		...
		foreach my $query (@{$p_queries}) {
			...
			my $agent_id = exists($fullans{'@id'}) ? $fullans{'@id'} : '-';
			...
			foreach my $fullrepo (@{$fullans{'repos'}}) {
				...
				if(defined($TAB)) {
					...
					# Synthetic
					unless(exists($p_ans->{'agent_id'})) {
						$p_ans->{'agent_id'} = $agent_id;
					}
			...
			if(defined($jsondir)) {
				my $partialJsonout = 'agent-'.$numAgent.'.json';
				push(@manifest,{
					'@id'	=>	$fullans{'@id'},
					'file'	=>	$partialJsonout
				});
