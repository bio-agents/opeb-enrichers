			...
			if($followProcessing && exists($repoData->{'links'}) && ref($repoData->{'links'}) eq 'HASH') {
				...
				if(exists($repoData->{'language'}) && defined($repoData->{'language'}) && length($repoData->{'language'}) > 0) {
					...
					if(exists($recognizedBuildSystemsByLang{$lcLanguage})) {
						$ans{'agent_buildSystem'} = $recognizedBuildSystemsByLang{$lcLanguage};
					}
				}
				...
				# Versions
				$ans{'agent_versions'} = \@versions  if(scalar(@versions) > 0);
				# credits
				$ans{'agent_developers'} = \@contributors  if(scalar(@contributors) > 0);
			}
		}
